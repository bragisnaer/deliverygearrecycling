...
Plans:
- [ ] 06-01: Audit trail schema — audit_log table, Postgres trigger covering all editable record types
- [ ] 06-02: Edit-in-place component — field-level editing, 48-hour window enforcement, edited indicator, edit history modal
- [ ] 06-03: Void policy — voided flag + reason, exclusion from calculations, reco-admin-only action
- [ ] 06-04: Processing report schema — wash and pack records, size buckets for clothing, facility and client scope
- [ ] 06-05: Wash and Pack forms — tablet-first, Danish labels, client/product/size-bucket inputs, two-tap access
- [ ] 06-06: Processing pipeline view — items at each stage per facility; visible to prison staff (own) and reco-admin (all)
- [ ] 06-07: Outbound dispatch schema and UI — dispatch records, packing list with SKU codes, status lifecycle
- [ ] 06-08: Prison dispatch view — dispatch history for own facility (prison staff read-only)
- [ ] 06-09: Traceability chain view — linked record view across all five pipeline stages
...
| 4. Pickup Booking and Transport Management | 1/10 | In Progress|  |
